...
public class EnchantGrassStomper extends EnchantmentBase {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        for (ItemStack stack : player.getArmorInventoryList()) {
            if (!stack.isEmpty() && player.getEntityWorld().isRemote && stack.getItem() instanceof ItemArmor && ((ItemArmor) stack.getItem()).getEquipmentSlot().ordinal() == 2)
                boots = stack;
        }


        if (!boots.isEmpty() && EnchantmentHelper.getMaxEnchantmentLevel(this, player) > 0) {
